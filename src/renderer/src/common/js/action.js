 ...
 * @LastEditors: 李星阳
 * @LastEditTime: 2023-08-13 22:09:07
 * @Description: 
 */
...
export default class Action {
    ...
    async initRecord(oAction){
        ...
        const {actionEnd} = this.oRecord; // 提前保存好上次行动结束时间
        ...
        if (oAction.ongoing){
            ...
            oAction.gapToPrev = 1 * ((actionBegin - actionEnd) / 1000).toFixed(2);
        }
        console.log('已初始化记录', oAction);
        this.oRecord = oAction;
    };
    // ▼保存
    async saveRecord(playEnd = () => {throw '请传入媒体当前时间点'}){
        ...
        let useToSave = structuredClone(this.oRecord);
        Object.assign(useToSave, { // 将记录补充完整（对下次生成记录有用处）
            duration,
            playEnd,
            actionEnd,
        });
        console.log('已“补全了”记录：', useToSave);
